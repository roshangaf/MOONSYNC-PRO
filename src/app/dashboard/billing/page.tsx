...
import { Plus, Trash2, FileText, Receipt, Download, Banknote, Calculator } from "lucide-react"
            ...
            <div className="p-4 bg-slate-50 rounded-2xl border border-dashed border-slate-300 space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <Input 
                    type="number" 
                    placeholder="Rate / Amount (NRS)" 
                    value={amount} 
                    onChange={(e) => setAmount(e.target.value)}
                    className="bg-white text-xs h-10 border-slate-200"
                  />
                  <Button onClick={addItem} className="h-10 px-6 font-bold">
                    Add Entry
                  </Button>
                </div>
              </div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="hover:bg-transparent border-none">
                        <TableHead className="text-[10px] font-black uppercase text-slate-400">Description of Service</TableHead>
                        <TableHead className="text-right text-[10px] font-black uppercase text-slate-400">Value (NRS)</TableHead>
                    ...
                    <TableBody>
                      {items.map((item) => (
                        <TableRow key={item.id} className="border-b-slate-100 hover:bg-slate-50/50">
                          ...
                          <TableCell className="text-right font-mono font-bold">{item.amount.toLocaleString()}.00</TableCell>
                          <TableCell>
